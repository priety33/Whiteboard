...
function createBox(){
  ...
  let initialX = null;
  let initialY = null;
  let isStickyDown = false; // true when we have clicked on sticky note and are moving it
...
function handleToolChange(tool) {
  if (tool == "pencil") {
    ...
    if (Activetool == "eraser") {
      ...
      ctx.globalCompositeOperation = "destination-out"; // rubbing, other way-> context.stroke= white
      ctx.lineWidth = inputs[0].value;
